...
namespace Game.Core.Tests
{
    public class CardSystemTests
    {
        ...
        public void RegisterTestAbilities()
        {
            ...
            db.Register(new AbilityDefinition { AbilityId = "armored", Trigger = AbilityTrigger.OnDamaged, Effect = AbilityEffect.ReduceDamage, Target = AbilityTarget.Self });
            db.Register(new AbilityDefinition { AbilityId = "thorns", Trigger = AbilityTrigger.OnDamaged, Effect = AbilityEffect.DealDamage, Target = AbilityTarget.Attacker });
            db.Register(new AbilityDefinition { AbilityId = "bounty", Trigger = AbilityTrigger.OnKill, Effect = AbilityEffect.GainGold, Target = AbilityTarget.Owner });
        ...
        public void TryCreate_DeepCopiesAbilitiesFromDefinition()
        {
            var state = new GameState(seed: 1);
            var def = Guy();
            def.Abilities.Add(new AbilityRef { Id = "armored", X = 2 });

            var card = CardFactory.Create(state, def, 0);

            Assert.AreEqual(1, card.Abilities.Count);
            Assert.AreEqual("armored", card.Abilities[0].Id);
