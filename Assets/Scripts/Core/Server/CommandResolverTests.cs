...
namespace Game.Core.Tests
{
    public class CommandResolverTests
    {
        ...
        public void StartGame_DealsStartingHandsAndEnergy()
        {
            ...
            {
                Assert.AreEqual(4, p.cardsInHand.Count, $"P{p.Id} hand size");
                Assert.AreEqual(6, p.Deck.Count, $"P{p.Id} deck after dealing (10 - 4)");
        ...
        public void StartGame_EmitsExpectedEventTypes()
        {
            ...
            Assert.AreEqual(8, events.OfType<CardDrawnEvent>().Count(), "4 per player");
        ...
        public void PlayCard_RejectedWhenTooExpensive()
        {
            ...
            Assert.AreEqual(4, p0.cardsInHand.Count, "hand untouched after rejection");
        }
        ...
        public void EndPhase_LastActionSlot_RunsCombatBlockAndSettles()
        {
            ...
            // energy block: cap grew 1 -> 2, refilled, one draw each
            foreach (var p in state.Players)
            {
                Assert.AreEqual(2, p.EnergyPerTurn, $"P{p.Id} cap grew");
                Assert.AreEqual(2, p.CurrentEnergy, $"P{p.Id} refilled");
            }
            Assert.AreEqual(p0Hand + 1, state.Players[0].cardsInHand.Count, "P0 drew");
            Assert.AreEqual(p1Hand + 1, state.Players[1].cardsInHand.Count, "P1 drew");
        ...
        public void DrawCard_EmptyDeckIsSafeNoOp()
        {
            ...
            // walk to a combat so StartNewEnergyBlock tries (and fails) to draw
            Submit(state, new EndPhaseCommand(0));
            Submit(state, new EndPhaseCommand(1));
            Submit(state, new EndPhaseCommand(1));
            ...
            Assert.AreEqual(4, state.Players[0].cardsInHand.Count, "hand unchanged");
        }
    }
}